...
MAX_EPOCHS = 15
PATIENCE = 3
CKPT = "./data/bigdata_S.pt"
PROGRESS = "./data/bigdata_S_progress.json"


def main():
    print("=" * 70)
    print("BIG-DATA S (resumable)")
    print("=" * 70)
    ...
    for name, freeze_after in [("prefix40", 40), ("full", None)]:
        m = OnlineLLMV3(vocab_size=tok.vocab_size, **CFG_S).to(device)
    ...
    print("\nS BIG-DATA:", {k: (round(v["bpc"], 3) if isinstance(v, dict) else v)
                            for k, v in row.items() if k in ("frozen", "prefix40", "full")})
    with open("./data/bigdata_S_results.json", "w") as f:
