...
**Qwen line (stopping b).** Qwen2.5-0.5B (Apache-2.0, 942 MB, new tokenizer/id-space so a separate line; MODEL_OVERRIDE hook, decoder needs the same env). One 28K-token segment: K1024 → 0.8559 @ 5.4 s (216 units); K2048 → **0.8442** @ 5.6 s (219 units, PEAK 5.32 GB — the crown pick, clean, no paging); K4096/PF4096 → 0.8391 @ 14.9 s but PEAK 9.11 GB pages over the 8 GB card (fwd 14.0 s thrash) — kept as the documented big-V VRAM wall, not the crown. Enablers: the chunk path (8.7 GB logits would OOM otherwise), the sparse path (17 GB dense blk avoided), and a deterministic alphabet clamp in uniform_cum_32 (floor auto-fits V; both sides agree with no new knobs). Stopping (b): 0.8442 < 0.9003 ✓.
