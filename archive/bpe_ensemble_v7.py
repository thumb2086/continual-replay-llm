...
from ac32 import (
    ...
    ArithmeticEncoder32, ArithmeticDecoder32, TOTAL32,
)
...
def main():
    print("=" * 70)
    print("SMOLLM2 FLOOR32 ENSEMBLE V7 ON ENWIK8")
    print("=" * 70)
            ...
            for k, (block, T0, NC, n_new) in enumerate(chunk):
                ...
                for t_idx in range(T0, T0 + NC):
                    ...
                    if len(hit):
                        ...
                        rank = int(np.where(rest == target)[0][0])
                        vprev = block[t_idx]
                        vprev2 = block[t_idx - 1] if t_idx > 0 else prev_tail
                        if USE_CACHE_S2:
    ...
    tag = (f"off{_off_mb}_lam{BIGRAM_LAMBDA}_gputopk{USE_GPU_TOPK}_bf{BATCH_FWD}"
           ...
           f"_f{FLOOR_FRAC:g}_s2{USE_CACHE_S2}")
    results = {
        "model": "SmolLM2-135M-floor32-ensemble-v7",
    ...
    with open(f"./data/smollm2_ensemble_v7_{tag}.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved to data/smollm2_ensemble_v7_{tag}.json")
