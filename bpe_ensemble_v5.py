"""SmolLM2 + bigram+trigram KN-interpolated ensemble (v5) on enwik8.

v5 = v4 plumbing + hierarchical cache:
  cache = wT*T + (1-wT)*wB*B        (trigram backs off to bigram)
  final = lam_eff*p + (1-lam_eff)*cache
  wT = tT/(tT+TRIGRAM_CONF), wB = tB/(tB+BIGRAM_CONF)
  lam_eff = 1-(1-LAMBDA)*(1-(1-wT)*(1-wB))
v3's lesson (pure trigram 0.986 < bigram 0.97) is respected: trigram
never stands alone, it only sharpens where it has evidence.
Knobs: USE_TRIGRAM=0 reproduces v4 exactly (validation).
"""
import torch
import numpy as np
import sys
...
def main():
    print("=" * 70)
    print("SMOLLM2 + KN-TRIGRAM ENSEMBLE V5 ON ENWIK8")
    print("=" * 70)
    ...
    tag = (f"off{_off_mb}_lam{BIGRAM_LAMBDA}_gputopk{USE_GPU_TOPK}_bf{BATCH_FWD}"
           f"_pf{PREFILTER}_tri{USE_TRIGRAM}_bc{BIGRAM_CONF}_tc{TRIGRAM_CONF}_k{TOP_K}")
    results = {
        "model": "SmolLM2-135M-KNtrigram-ensemble-v5",
    ...
    with open(f"./data/smollm2_ensemble_v5_{tag}.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved to data/smollm2_ensemble_v5_{tag}.json")
