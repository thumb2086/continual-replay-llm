...
REPLAY_SIZES = [0, 1, 2, 4, 8]


def sample_replay(tokenizer, pool, device, batch_size):
    picks = [pool[i] for i in np.random.choice(len(pool), batch_size, replace=False)]
    return make_batch(tokenizer, picks, device)


def main():
    print("=" * 70)
    print("REPLAY SIZE SWEEP (EWC off)")
    print("=" * 70)
    ...
    print(f"Replay sizes: {REPLAY_SIZES}")
    ...
    results = {"start": {"a": start_a, "b": start_b}, "replay_sizes": {}}
    ...
    for size in REPLAY_SIZES:
        ...
        for epoch in range(ONLINE_EPOCHS):
            ...
            for chunk in topic_b[:ONLINE_CHUNKS]:
                ...
                if size == 0:
                    total_task += model.pretrain_step(tokens, opt)
                else:
                    replay = sample_replay(tok, topic_a, device, size)
                    task_l, _, _ = model.online_step(tokens, opt, replay, 0)
                    total_task += task_l
    ...
    with open("./data/replay_sweep.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\nSaved to data/replay_sweep.json")
