...
import os as _os
COSMO_SAMPLES = int(_os.environ.get("COSMO_SAMPLES", "175"))
RESULT_JSON = _os.environ.get("RESULT_JSON", "./data/mixed_S_balanced_results.json")
...
def main():
    ...
    print("\n[5/5] Evaluating on enwik8 sample (FULL verify)...")
    from real_compression import (
        gpu_quantize, ArithmeticEncoder, ArithmeticDecoder,
    )
    with open("./data/cloud/enwik8", "rb") as f:
        raw = f.read(100 * 1024)
    etext = raw.decode("utf-8", errors="ignore")
    echunks = [etext[i:i + 128] for i in range(0, len(etext) - 128, 128)]
    unk = sum(1 for c in etext[:len(echunks) * 128] if c not in tok.char2idx)
    torch.cuda.synchronize()
    t = time.time()
    ebits, echars, eok, efail = 0, 0, 0, 0
    model.eval()
    with torch.no_grad():
        for ci in range(0, len(echunks), 32):
            block = echunks[ci:ci + 32]
            id_lists = []
            for c in block:
                ids = tok.encode(c)
                if len(ids) >= 3:
    ...
    results["enwik8"] = {"bpc": ebits / echars, "chars_per_s": echars / dt,
                         ...
                         "verified_ok": eok, "verified_fail": efail}
    print(f"  enwik8: {ebits/echars:.3f} bpc, UNK {unk/(len(echunks)*128)*100:.2f}%, "
          f"verified {eok}/{eok+efail}", flush=True)
