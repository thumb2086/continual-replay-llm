"""SmolLM2 + KN-trigram ensemble (v6: overlap scoring + real lossless) on enwik8.

v6 = v5 math + two structural fixes:
1. Scoring overlap (OVERLAP, default 1024): each segment's forward input
   is tail(OVERLAP coded tokens) + new(BLOCK_TOKENS-OVERLAP). Early-block
   tokens get real left context instead of cold start.
2. REAL lossless bitstream (fixes latent v2-v5 bug): v2-v5 never coded
   file token 0 or any block-boundary token as a target -- their 200-spot
   verify only sampled coded pairs, so it never caught the 4 missing
   tokens. v6 codes every token >= 1 exactly once (first pair of each
   overlap segment is (prev_tail -> new[0])), plus ids[0] once via
   uniform(V) (~16 bits one-time). Verify additionally decodes token 0
   and all segment-boundary targets.
Knobs: OVERLAP=0 reproduces v5 pair structure (parity check; still
carries the boundary hole -- use OVERLAP>=1 for real runs).
"""
import torch
import numpy as np
import sys
...
from ac32 import (
    uniform_cum_32, nb_encode_count_32,
...
OVERLAP = int(os.environ.get("OVERLAP", "1024"))
...
def main():
    print("=" * 70)
    print("SMOLLM2 OVERLAP-KN ENSEMBLE V6 ON ENWIK8")
    print("=" * 70)
    ...
    with torch.no_grad():
        ...
        while si < len(segs):
            ...
            for k, (block, T0, NC, n_new) in enumerate(chunk):
                ...
                # v6 also force-verifies every segment-boundary target
                # (t_idx == T0 on overlap segs) + file token 0 (separate).
                base = len(stored) - NC
                for t_idx in range(T0, T0 + NC):
                    gpos = coded_count + (t_idx - T0)
                    is_bnd = OVERLAP > 0 and t_idx == T0
                    if (gpos % 130 != 0 and not is_bnd) or n_verify_ok + n_verify_fail >= 220:
                        continue
                    v_topk, v_p = stored[base + (t_idx - T0)]
                    target = block[t_idx + 1]
                    ...
                    if got == target:
                        ...
                        n_verify_fail += 1
                        print(f"  FAIL at coded-pos {gpos}, pos {t_idx}")
            ...
            if done_n % 2 == 0 or done_n == len(segs):
                print(f"  ... seg {done_n}/{len(segs)}, escapes: {n_escapes}, "
                      f"bigram keys: {len(bi_counts)}, coded: {coded_count}", flush=True)
    ...
    results = {
        "model": "SmolLM2-135M-overlapKN-ensemble-v6",
    ...
    with open(f"./data/smollm2_ensemble_v6_{tag}.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved to data/smollm2_ensemble_v6_{tag}.json")
